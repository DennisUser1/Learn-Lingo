...
const TeacherInfo: React.FC<TeacherInfoProps> = ({ teacher }) => {
    return (
      <div className="flex justify-between">
        ...
        <ul className="flex flex-col tablet:flex-row items-start gap-1 tablet:gap-4 max-tablet:w-full text-base font-medium h-max desktop:mr-[90px] ">
          <li className="flex items-center gap-1">
            <svg className="w-4 h-4 fill-none stroke-black dark:stroke-white">
              <use href={`/sprite.svg#icon-book-open`} />
            </svg>
            Lessons online
          </li>
  
          <li className="max-tablet:hidden w-0.5 h-4 bg-black dark:bg-white opacity-20 self-center"></li>
